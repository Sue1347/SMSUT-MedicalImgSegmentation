...
from network.ugan import UGAN, Discriminator, UGANnce
...
class UGANShp0Trainer(BaseTrainer):
    def __init__(self, phase, args):
        ...
        super(UGANShp0Trainer, self).__init__(phase, args)

    def build_network(self):
        self.net = UGANnce(cfg.img_channels, cfg.n_label + 1, cfg.n_modal, cfg.base_width)
        count_param_number(self.net, verbose=True, logger=self)
        self.net.to(self.device)

        self.criterionNCE = []
        for nce_layer in cfg.nce_layers:
            self.criterionNCE.append(PatchNCELoss(cfg.batch_size).to(self.device))

        self.D = Discriminator(cfg.input_size, cfg.n_modal, cfg.base_width,
                               max_width=256 if cfg.base_width == 16 else 512)
    ...
    def load_model(self, model_idx, which_ckpt):
        ...
        # F_path = pjoin(self.expr_root, model_idx, 'ckpt', f'{which_ckpt}_F.ckpt')
        D_path = pjoin(self.expr_root, model_idx, 'ckpt', f'{which_ckpt}_D.ckpt')

        self.net = UGANnce(cfg.img_channels, cfg.n_label + 1, cfg.n_modal, cfg.base_width)  # 实例化自己的模型；

        checkpoint = torch.load(G_path, map_location=lambda storage, loc: storage)  # 加载模型文件，pt, pth 文件都可以；
        if torch.cuda.device_count() > 1:
            # 如果有多个GPU，将模型并行化，用DataParallel来操作。这个过程会将key值加一个"module. ***"。
            self.net = nn.DataParallel(self.net)
        self.net.load_state_dict(checkpoint)  # 接着就可以将模型参数load进模型。
    ...
    def validate_epoch(self, loader, npys, meter=None, save_path=None):
        self.net.eval()
        self.net.to(self.device) # add for no reason by Sue
        ...
        with torch.no_grad():
            for x_real, y_real, mdl, inm in loader:
                ...
                y_fake, x_fake = self.net(x_real, val_phase=True)
                if b != cfg.batch_size:
...
if __name__ == '__main__':
    ...
    trainer = UGANShp0Trainer(args.phase, args)
    if args.phase == 'train':
        trainer.fit('inTurn')
    else:
